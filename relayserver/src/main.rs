...
async fn main() {
    ...
    // Database state
    let connection_string = SETTINGS.database.to_connection_string();
    info!(message = "opening database", connection_string = %connection_string);
    let db = Database::try_new(&connection_string)
        .await
